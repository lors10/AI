...
class FifteenPuzzle():
    def __init__(self, input_str):
        pttn = re.compile("(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)\s(\d+)")
        result = pttn.match(input_str)
        if result is not None:
            s = result.groups()

            self.state = [[int(s[0]), int(s[1]), int(s[2]), int(s[3])],
                          [int(s[4]), int(s[5]), int(s[6]), int(s[7])],
                          [int(s[8]), int(s[9]), int(s[10]), int(s[11])],
                          [int(s[12]), int(s[13]), int(s[14]), int(s[15])]]
            print(self.state)

        else:
            raise InputError("Improperly formatted 15-puzzle")
...
def main():
    ...
    h = int(sys.argv[1])
    if h == 1:
        heuristic = FifteenPuzzle.manhatten_distance
    elif h == 2:
        heuristic = FifteenPuzzle.tile_switches_remaining
    else:
        raise InputError("Heuristic argument must be 1 or 2")
    if len(sys.argv) == 3:
        o = int(sys.argv[2])
        if o == 0:
            output = FifteenPuzzle.action_sequence
        elif o == 1:
            output = FifteenPuzzle.state_transition
        else:
    ...
    initial = FifteenPuzzle(input_i)
    goal = FifteenPuzzle(input_f)
